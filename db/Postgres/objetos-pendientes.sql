-- =====================================================================
-- EduSphere LMS — Objetos de BD que invoca el backend
-- Procedimientos, función y vista para operaciones y reportes.
-- Cargar en la BD: psql -U admin -d edusphere_db -f db/Postgres/objetos-pendientes.sql
-- =====================================================================

-- ---------------------------------------------------------------------
-- OC-01: sp_inscribir_estudiante(p_estudiante_id, p_curso_id)
-- Inscribe a un estudiante en un curso publicado, evitando duplicados.
-- Lo invoca: src/models/operaciones.model.js -> CALL sp_inscribir_estudiante($1,$2)
-- ---------------------------------------------------------------------
...
BEGIN
    ...
    -- El curso debe existir; tomamos su estado y el precio efectivo (con descuento si aplica).
    SELECT c.categoria::text, COALESCE(c.precio_descuento, c.precio)
    ...
    -- Solo se permite inscribirse a cursos publicados.
    IF v_estado_curso <> 'publicado' THEN
        RAISE EXCEPTION 'El curso no está publicado (estado actual: %)', v_estado_curso
            USING ERRCODE = 'check_violation';
    END IF;

    -- No permitir doble inscripción al mismo curso.
    IF EXISTS (
...
-- ---------------------------------------------------------------------
-- OC-02: sp_emitir_certificado(p_inscripcion_id)
-- Emite el certificado de una inscripción completada (una sola vez).
-- Lo invoca: src/models/operaciones.model.js -> CALL sp_emitir_certificado($1)
-- ---------------------------------------------------------------------
...
BEGIN
    -- Traer estado de la inscripción, si el curso permite certificado y el estudiante.
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'La inscripción % no existe', p_inscripcion_id;
    END IF;

    -- Solo se certifica una inscripción completada.
    IF v_estado <> 'completado' THEN
        RAISE EXCEPTION 'No se puede certificar: la inscripción no está completada (estado: %)', v_estado
            USING ERRCODE = 'check_violation';
    END IF;

    -- El curso debe permitir certificado.
    IF NOT v_permite_cert THEN
        RAISE EXCEPTION 'El curso no otorga certificado'
            USING ERRCODE = 'check_violation';
    END IF;

    -- Evitar emitir dos veces (la tabla ya tiene UNIQUE en inscripcion_id, esto da mejor mensaje).
    IF EXISTS (SELECT 1 FROM certificados WHERE inscripcion_id = p_inscripcion_id) THEN
        RAISE EXCEPTION 'El certificado de esta inscripción ya fue emitido'
            USING ERRCODE = 'unique_violation';
    END IF;

    -- Código único derivado del id de la inscripción (que ya es único).
...
-- ---------------------------------------------------------------------
-- RC-04: fn_ingresos_instructor(p_instructor_id, p_desde, p_hasta)
-- Ingresos del instructor por curso en un rango de fechas.
-- ingreso_neto = monto_pagado descontando la comisión aplicada de la plataforma.
-- Lo invoca: src/models/reportes.model.js -> SELECT * FROM fn_ingresos_instructor($1,$2,$3)
-- ---------------------------------------------------------------------
...
-- Catálogo público: solo cursos en estado 'publicado', con su categoría e instructor.
-- Lo invoca: src/models/reportes.model.js -> SELECT * FROM vw_catalogo_cursos_publicados
-- ---------------------------------------------------------------------
